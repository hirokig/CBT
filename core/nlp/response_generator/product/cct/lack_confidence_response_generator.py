...
class LackConfidenceResponseGenerator(BaseResponseGenerator):
    """
    This class creates responses for users who seems unconfident
    """
    ...
    def __create_response_for_lack_of_confidence(self, df):
        if Df_util.anything_isin(["whore", "slut", "bitch"], df["base_form"]):
            cmp_list, labeling_list, comfort_list = self.__create_responses_for_bitch()
        elif Df_util.anything_isin(["fat", "stupid", "ugly", "burden", "not good enough"], df["base_form"]):
            cmp_list, labeling_list, comfort_list = self.__create_responses_for_fat()
        elif Df_util.anything_isin(["bother"], df["base_form"]):
            cmp_list, labeling_list, comfort_list = self.__create_responses_for_bother()
        elif Df_util.anything_isin(["hate"], df["base_form"]):
            cmp_list, labeling_list, comfort_list = self.__create_responses_for_hate()
        else:
            raise UnknownError
    ...
    def __create_responses_for_bitch(self):
        ...
        return cmp_list, labeling_list, comfort_list

    def __create_responses_for_fat(self):
        cmp_list = [
            ["I’m sorry that you are feeling like that about yourself.."],
            ["it is sad to hear that you feel that way now.."],
        ...
        comfort_list = [
            ["let me just be with you now. i love you the way you are😊"],
            ["idk if i can help you much but let me be here with you☺️"],
    ...
    def __create_responses_for_bother(self):
        cmp_list = [
            ["I’m sorry that you are feeling like that about yourself.."],
            ["it is sad to hear that you feel that way now.."],
            ["thats sad to feel like that..."]
    ...
    def __create_responses_for_hate(self):
        cmp_list = [
            ["I’m sorry that you are feeling like that about yourself.."],
            ["it is sad to hear that you feel that way now.."],
            ["omg..."]
