...
class QuestionResponseGenerator(BaseResponseGenerator):
    """
    This class creates responses for question
    To see the difference between question responses, see question.py
    """
    def __call__(self):
        responses = []
        ...
        if number_of_question_so_far == 0:
            ...
            question_type = question.DefineQuestionType(
                self.message.text_df[self.message.text_df.sidx.isin(question_sidx)]).categorize_by_leading_word()
